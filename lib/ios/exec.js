...
function iOSExec() {
    ...
    if (cordova.commandQueue.length == 1 && !cordova.commandQueueFlushing) {
        if (bridgeMode) {
            ...
            execXhr.open('HEAD', "file:///exec", true);
...
// Bridge mode values:
// 0 - iframe nav
// 1 - XHR with no embedded JSON
// 2 - XHR with embedded JSON
// 3 - XHR with embedded JSON only when it is faster to do so
iOSExec.setBridgeMode = function(mode) {
